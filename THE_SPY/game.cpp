...
int game::ShowOptions()
{
	...
	saveOptionButtons[0] = LoadText(font2, slot[0].c_str(), {200, 200, 0});
	saveOptionButtons[1] = LoadText(font2, slot[1].c_str(), {200, 200, 0});
	saveOptionButtons[2] = LoadText(font2, slot[2].c_str(), {200, 200, 0});
	in.close();
	...
	while( !inputTaken )
	{
		...
		if( renderText )
		{
			if( inputText != "" )
			{
				...
				textureG = LoadText(font2, inputText.c_str(), {0, 255, 0});
			}
			else
			{

				//gInputTextTexture.loadFromRenderedText( " ", textColor );
				textureG = LoadText(font2, " ", {0, 255, 0});
			}
		}
