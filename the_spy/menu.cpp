...
void Module::LoadMedia()
{
	...
	forContinueButton.x = 450;
	forContinueButton.y = 160;
	forContinueButton.w = 130;
	forContinueButton.h = 40;

	playButton = LoadText(font, "New Game", textColor);

	forPlayButton.x = 450;
	forPlayButton.y = forContinueButton.y + 60;
	forPlayButton.w = 130;
	forPlayButton.h = 40;

	instructionsButton = LoadText(font, "Instructions", textColor);
	
	forInstuctionButton.x = 450;
	forInstuctionButton.y = forPlayButton.y + 60;
	forInstuctionButton.w = 130;
	forInstuctionButton.h = 40;


	scoreButton = LoadText(font, "Highest Scores", textColor);

	forScoreButton.x = forPlayButton.x;
	forScoreButton.y = forInstuctionButton.y + 60;
	forScoreButton.h = forPlayButton.h;
	forScoreButton.w = 130;
	

	exitButton = LoadText(font, "Quit", textColor);

	forExitButton.x = forPlayButton.x;
	forExitButton.y = forScoreButton.y + 60;
...
void Module::LoadScoreMedia()
{
	...
	SDL_Color greenColor = { 200,200,0 };
	...
	for (int i = 1; i <= 5; i++)
	{
		char s[100];
		sprintf(s, "%d.", i);
		scoreNum[i - 1] = LoadText(font, s, greenColor);
	}
	...
	forWoodBack.x = 1;
	forWoodBack.y = 1;
	forWoodBack.w = SCREEN_WIDTH;
	forWoodBack.h = SCREEN_HEIGHT;
...
void Module::LoadResumeMedia()
{
	...
	forResumeButton.x = 430;
	forResumeButton.y = 240;
	forResumeButton.w = 100;
...
{
	 window = SDL_CreateWindow("SPY", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
