...
bool GameOverScreen()
{
	...
	while (event->type != SDL_KEYDOWN && event->type != SDL_QUIT && level<=8)
	{
		SDL_WaitEvent(event);
	}

	//Quits the Game
	return event->type == SDL_QUIT;//edit:add then deleted  //edit:add  event->key.keysym.sym == SDLK_ESCAPE || ;//24_11.30pm
}
...
int main(int argc, char *argv[])
{
	...
	else 
	{
		...
		//Game Running Loop
		while (!quit && gameEvent->type != SDL_QUIT && curGameState >= Menu && level<8)
		{
			//Front End Menu Loop
			while(!quit && gameEvent->type != SDL_QUIT && curGameState == Menu && level<8)
			{
			...
			//Gameplay Loop
			while (!quit && gameEvent->type != SDL_QUIT && curGameState == InGame && level<8) 
			{
				//Do something when there is an event
